...
#include "ijksdl/android/ijksdl_codec_android_mediacodec.h"
...
#define AMC_NORMAL_TIMEOUT_US   (100 * 1000)
#define AMC_FAST_TIMEOUT_US     (10  * 1000)
#define MAX_FAKE_FRAMES (2)

typedef struct AMC_Buf_Out {
    int port;
    //int acodec_serial;delete
...
typedef struct IJKFF_Pipenode_Opaque {
    ...
    int                       buffer_stride;
    int                       buffer_slice_height;
    int                       buffer_color_format;
...
//        ALOGD("create a dummy media codec ");
//    } else {
...
//    }
    ...
    if (acodec) {
        ...
        */
        ...
        ALOGE("we donot get a mediacodec ");
    }

    opaque->frame_width  = opaque->codecpar->width;
    opaque->frame_height = opaque->codecpar->height;

    ALOGI("[rotation] frame_width(%d) frame_height(%d)", opaque->frame_width, opaque->frame_height);
...
static int recreate_format_l(JNIEnv *env, IJKFF_Pipenode *node)
{
    ...
    if (opaque->codecpar->extradata && opaque->codecpar->extradata_size > 0) {
        if ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 && opaque->codecpar->extradata[0] == 1)
            ...
            if(opaque->decoder->avctx->codec_id == AV_CODEC_ID_H264) {
                ...
                if (0 != convert_hevc_nal_units(opaque->decoder->avctx->extradata, opaque->decoder->avctx->extradata_size,
                                                convert_buffer, convert_size,
                                                &sps_pps_size, &opaque->nal_size)) {
    ...
    ALOGI("[raw] %d auto-rotate:%d", opaque->weak_vout->get_decode_raw_data_enable(opaque->weak_vout), ffp->mediacodec_auto_rotate);
...
static int configure_codec_l(JNIEnv *env, IJKFF_Pipenode *node, jobject new_surface)
{
    ...
    if (new_surface) {
        ...
        if (SDL_JNI_CatchException(env) || !opaque->jsurface)
            goto fail;
    } else {
        opaque->jsurface = NULL;
        //return 0;//donot create codec when surface is null;
    }
    if(prev_jsurface!=NULL)
    ...
    if (SDL_AMediaCodec_isConfigured(opaque->acodec)) {
        ...
        if (opaque->acodec) {
            if (SDL_AMediaCodec_isStarted(opaque->acodec)) {
                //SDL_VoutAndroid_invalidateAllBuffers(opaque->weak_vout); delete not use buffer queue
            ...
            if (opaque->quirk_reconfigure_with_new_codec) {
                ...
                SDL_LockMutex(opaque->weak_vout->mutex);
                SDL_VoutAndroid_setAMediaCodecWithoutLock(opaque->weak_vout, NULL);
                SDL_AMediaCodec_decreaseReferenceP(&opaque->acodec);
                SDL_UnlockMutex(opaque->weak_vout->mutex);
    ...
    int decode_raw_data = opaque->weak_vout->get_decode_raw_data_enable(opaque->weak_vout);
    ALOGI("[rotation] frame_width(%d) frame_height(%d) decode_raw_data(%d)", opaque->frame_width, opaque->frame_height, decode_raw_data);
    if (decode_raw_data) {
        ALOGI("configure codec no surface");
        amc_ret = SDL_AMediaCodec_configure_surface(env, opaque->acodec, opaque->input_aformat, NULL, NULL, 0);
    } else {
        ALOGI("configure codec with surface");
...
static int reconfigure_codec(JNIEnv *env, IJKFF_Pipenode *node)
{
    ...
    int ret = configure_codec_l(env, node);
    ...
    SDL_AMediaCodecBufferInfo *buffer_info)
{
    ...
    if(opaque->weak_vout->get_decode_raw_data_enable(opaque->weak_vout)) {
        int color_format = opaque->buffer_color_format;
        int stride       = opaque->buffer_stride;
        int slice_height = opaque->buffer_slice_height;
        int ffmpeg_format = 0;
        int change_format = 0;

        AVFrame *src_frame = av_frame_alloc();
        AVFrame *output_frame = src_frame;

        if(color_format == AMEDIACODEC__OMX_COLOR_FormatYUV420SemiPlanar || color_format == AMEDIACODEC__OMX_COLOR_FormatYUV420PackedSemiPlanar) {
            ffmpeg_format = AV_PIX_FMT_NV12;
            change_format = 1;
        }else {
            ffmpeg_format = AV_PIX_FMT_YUV420P;
        }

        avpicture_fill((AVPicture *) src_frame, buffer_info->bufferptr, ffmpeg_format, stride, slice_height);

        if (change_format) {
            output_frame = av_frame_alloc();

            int before_pic_size = avpicture_get_size(AV_PIX_FMT_YUV420P, opaque->codecpar->width, opaque->codecpar->height);
            buffer_info->buffer_change_format_ptr = (uint8_t *) av_malloc(before_pic_size);
            avpicture_fill((AVPicture *) output_frame, buffer_info->buffer_change_format_ptr, AV_PIX_FMT_YUV420P, opaque->codecpar->width, opaque->codecpar->height);

            uint8_t *srcSlice[8] = {src_frame->data[0], src_frame->data[1], src_frame->data[2], NULL, NULL, NULL, NULL, NULL};
            int srcStride[8] = {src_frame->linesize[0], src_frame->linesize[1], src_frame->linesize[2], 0, 0, 0, 0, 0};
        ...
        frame->data[0] = output_frame->data[0];
        frame->data[1] = output_frame->data[1];
        frame->data[2] = output_frame->data[2];
        frame->linesize[0] = output_frame->linesize[0];
        frame->linesize[1] = output_frame->linesize[1];
        frame->linesize[2] = output_frame->linesize[2];

        av_frame_free(&src_frame);
        if (change_format)
            av_frame_free(&output_frame);
    }
    frame->opaque = SDL_VoutAndroid_obtainBufferProxy(opaque->weak_vout, output_buffer_index, buffer_info);
    if (!frame->opaque)
        goto fail;
    *got_frame = 1;
    return 0;
...
static int feed_input_buffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        do {
            ...
            if (ffp_packet_queue_get_or_buffering(ffp, d->queue, false, &pkt, &d->pkt_serial, &d->finished) < 0) {
                ret = -1;
                goto fail;
            }
        ...
        if (opaque->ffp->vtb_handle_resolution_change &&
            ...
            if (side_data && side_data_size > 7) {
                ALOGE("[header] side data change size(%d)", side_data_size);
                int got_picture = 0;
                AVFrame* frame = av_frame_alloc();
                AVDictionary *codec_opts = NULL;
                const AVCodec *codec = opaque->decoder->avctx->codec;
                AVCodecContext* old_avctx = opaque->decoder->avctx;
                AVCodecContext* new_avctx = avcodec_alloc_context3(codec);
                int change_ret = 0;
                if (!new_avctx)
                    return AVERROR(ENOMEM);
//                avcodec_parameters_to_context(new_avctx, opaque->codecpar);
                copy_codec_parameters(new_avctx, old_avctx);
                av_freep(&new_avctx->extradata);
                new_avctx->extradata = av_mallocz(side_data_size + AV_INPUT_BUFFER_PADDING_SIZE);
                if (!new_avctx->extradata) {
                    avcodec_free_context(&new_avctx);
                    return AVERROR(ENOMEM);
                }
                memcpy(new_avctx->extradata, side_data, side_data_size);
                new_avctx->extradata_size = side_data_size;
                ...
                if (change_ret < 0) {
                    avcodec_free_context(&new_avctx);
                    return change_ret;
                } else if (old_avctx->width != new_avctx->width ||
                            old_avctx->height != new_avctx->height) {
                    ALOGE("[header] side data change 4 got_picture(%d) ret(%d) new_width(%d) new_height(%d) ", got_picture, ret, new_avctx->width, new_avctx->height);
//                  avcodec_parameters_from_context(opaque->codecpar, new_avctx);
                    copy_codec_parameters(opaque->decoder->avctx, new_avctx);
    ...
    if (d->pkt_temp.data) {
        ...
        if (ffpipeline_is_surface_need_reconfigure(pipeline)) {
            ...
            new_surface = ffpipeline_get_surface_as_global_ref(env, pipeline);
            if (!opaque->aformat_need_recreate &&
                ...
                SDL_JNI_DeleteGlobalRefP(env, &new_surface);
                ...
                ALOGI("%s: reconfigure codec \n", __func__);
                opaque->acodec_reconfigure_request = true;
                SDL_LockMutex(opaque->acodec_mutex);
                ret = configure_codec_l(env, node, new_surface);
                ...
                while (!q->abort_request &&
                    ...
                    AMCTRACE("AMEDIACODEC__INFO_INPUT_WAIT\n");
                }
        ...
        //ALOGD("try we get a packet for mediacodec %d",input_buffer_index);
        if (input_buffer_index < 0) {
            if (SDL_AMediaCodec_isInputBuffersValid(opaque->acodec)) {
                ...
                // enqueue fake frame
                //ALOGD("enqueue fake frame");
                queue_flags |= AMEDIACODEC__BUFFER_FLAG_FAKE_FRAME;
                copy_size    = d->pkt_temp.size;
            }
        } else {
            //SDL_AMediaCodecFake_flushFakeFrames(opaque->acodec); todo delete frist

            copy_size = SDL_AMediaCodec_writeInputData(opaque->acodec, input_buffer_index, d->pkt_temp.data, d->pkt_temp.size);
            //ALOGD("codecw write buffer copysize is  %d",copy_size);
            if (!copy_size) {
                ALOGE("%s: SDL_AMediaCodec_writeInputData failed\n", __func__);
                ret = 0;
                goto fail;
            }
...
static int enqueue_thread_func(void *arg)
{
    ...
    while (!q->abort_request && !opaque->abort) {
        ret = feed_input_buffer(env, node, timeUs, &dequeue_count);
        if (ret != 0) {
            goto fail;
        }
    }
...
static int drain_output_buffer_l(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, int *got_frame)
{
    ...
    //ALOGI("stream_open--->SDL_AMediaCodec_dequeueOutputBuffer");
    output_buffer_index = SDL_AMediaCodec_dequeueOutputBuffer(opaque->acodec, &bufferInfo, timeUs, opaque->weak_vout->get_decode_raw_data_enable(opaque->weak_vout));//not fake
    //ALOGD("try drain_output_buffer index is %d",output_buffer_index);
    if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        ...
        ALOGE("AMEDIACODEC__INFO_OUTPUT_FORMAT_CHANGED\n");
        SDL_AMediaFormat_deleteP(&opaque->output_aformat);
        ...
        if (opaque->output_aformat) {
            ...
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "color-format",   &opaque->buffer_color_format);

            SDL_AMediaFormat_getInt32(opaque->output_aformat, "stride",         &opaque->buffer_stride);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "slice-height",   &opaque->buffer_slice_height);
            ...
            ALOGI(
                ...
                ,
                width, height,
                SDL_AMediaCodec_getColorFormatName(opaque->buffer_color_format), opaque->buffer_color_format,
                opaque->buffer_stride,
                opaque->buffer_slice_height,
                crop_left, crop_top, crop_right, crop_bottom);

            if(opaque->weak_vout->get_decode_raw_data_enable(opaque->weak_vout)
             && opaque->buffer_color_format != AMEDIACODEC__OMX_COLOR_FormatYUV420Planar
             && opaque->buffer_color_format != AMEDIACODEC__OMX_COLOR_FormatYUV420PackedPlanar
             && opaque->buffer_color_format != AMEDIACODEC__OMX_COLOR_FormatYUV420SemiPlanar
             && opaque->buffer_color_format != AMEDIACODEC__OMX_COLOR_FormatYUV420PackedSemiPlanar) {
                ret = -1;
                ALOGE("exit mediacodec for not support color format (%d)", opaque->buffer_color_format);
                goto fail;
            }
        }
        // continue;
        ...
        ALOGI("AMEDIACODEC__INFO_OUTPUT_THREAD WAIT\n");
        ...
        if (opaque->n_buf_out) {
            ...
            if (opaque->off_buf_out < opaque->n_buf_out) {
                ...
                // buf_out->acodec_serial = SDL_AMediaCodec_getSerial(opaque->acodec); delete
                ...
                if (pts < buf_out->pts) {
                    ret = amc_fill_frame(node, frame, got_frame, output_buffer_index, &bufferInfo);
                    ...
                    for (i = opaque->off_buf_out - 1; i >= 0; i--) {
                        ...
                        if (pts > buf_out->pts) {
                            ret = amc_fill_frame(node, frame, got_frame, buf_out->port, &buf_out->info);
                            opaque->last_queued_pts = buf_out->pts;
                            //ALOGE("pts2 = %f", buf_out->pts);
...
static int func_run_sync(IJKFF_Pipenode *node)
{
    ...
    if (!opaque->acodec) {
        ALOGE("use amediacodec ,but acodec is null  ");
    ...
    while (!q->abort_request) {
        //timeUs = opaque->acodec_first_dequeue_output_request ? 0 : AMC_OUTPUT_TIMEOUT_US;
        got_frame = 0;
        ret = drain_output_buffer(env, node, timeUs, &dequeue_count, frame, &got_frame);
        //ALOGD("done drain output buffer ret=%d,got_frame=%d",ret ,got_frame);
        ...
        if (ret != 0) {
            ret = -1;
            if (got_frame && frame->opaque&&opaque->acodec)
                SDL_VoutAndroid_releaseBufferProxy_Vout((SDL_AMediaCodecBufferProxy **)&(frame->opaque),opaque->weak_vout,  false);
            goto fail;
        }
        if (got_frame) {
           // ALOGD("done drain output buffer ret=%d,got_frame=%d and we try to quue picture",ret ,got_frame);
            duration = (frame_rate.num && frame_rate.den ? av_q2d((AVRational){frame_rate.den, frame_rate.num}) : 0);
            pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
            ret = ffp_queue_picture(ffp, frame, pts, duration, av_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
            if(ret==0&&is->first_frame_time<=0){
                  ...
                  timeUs=AMC_NORMAL_TIMEOUT_US;
            }
            if (ret) {
                if (frame->opaque&&opaque->acodec)
                    SDL_VoutAndroid_releaseBufferProxy_Vout((SDL_AMediaCodecBufferProxy **)&(frame->opaque),opaque->weak_vout,  false);
            }
            av_frame_unref(frame);
        }
...
fail:
    ALOGI("MediaCodec: %s: try exit: %d", __func__, ret);
    av_frame_free(&frame);
    //SDL_AMediaCodecFake_abort(opaque->acodec);
    if (opaque->n_buf_out) {
    ...
    if (opaque->weak_vout->get_decode_raw_data_enable(opaque->weak_vout)) {
        frame_queue_unref_items(&ffp->is->pictq);
    }
    if (opaque->acodec) {
        //SDL_VoutAndroid_invalidateAllBuffers(opaque->weak_vout);delete not use bufferqueue
        SDL_LockMutex(opaque->acodec_mutex);
        SDL_AMediaCodec_stop(opaque->acodec);
        SDL_UnlockMutex(opaque->acodec_mutex);
        SDL_LockMutex(opaque->weak_vout->mutex);
...
IJKFF_Pipenode *ffpipenode_create_video_decoder_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout)
{
    ...
    case AV_CODEC_ID_H264:
        ...
        ffp->vtb_handle_resolution_change = 1;
        break;
    ...
    case AV_CODEC_ID_HEVC:
        switch (opaque->codecpar->profile) {
            case FF_PROFILE_HEVC_MAIN:
                ALOGI("%s: MediaCodec: HEVC_MAIN: enabled\n", __func__);
                break;
            case FF_PROFILE_HEVC_REXT:
                ALOGI("%s: MediaCodec: HEVC_REXT: disabled\n", __func__);
                goto fail;
                break;
            case FF_PROFILE_HEVC_MAIN_10:
                ALOGW("%s: MediaCodec: HEVC_MAIN_10: disabled\n", __func__);
                goto fail;
            case FF_PROFILE_HEVC_MAIN_STILL_PICTURE:
                ALOGW("%s: MediaCodec: HEVC_MAIN_STILL_PICTURE: disabled\n", __func__);
                goto fail;
            default:
                ALOGW("%s: MediaCodec: (%d) unknown profile: c\n", __func__, opaque->codecpar->profile);
                goto fail;
        }
    ...
    default:
        ALOGE(" try use javamediacodec decode not a h264 stream" );
        goto fail;
    }
    ...
    SDL_AMediaFormat_Opaque* media_opaque = opaque->input_aformat->opaque;
    opaque->mcc.mediaformat=media_opaque->android_media_format;
    if (!ffpipeline_select_mediacodec(pipeline, &opaque->mcc) || !opaque->mcc.codec_name[0]) {
    ...
    SDL_JNI_DeleteGlobalRefP(env, &jsurface);
    if (ret != 0)
        goto fail;
    ...
    ALOGD("ffpipenode_create_video_decoder_from_android_mediacodec() success \n");
    timeUs=AMC_FAST_TIMEOUT_US;
    return node;
fail:
...
int copy_codec_parameters(AVCodecContext* dst, AVCodecContext* src){
    ...
    ALOGI("copy_codec_parameters type(%d) codec_id(%d) tag(%d) bit_rate(%d)", dst->codec_type, dst->codec_id, dst->codec_tag, dst->bit_rate);

    switch (dst->codec_type) {
        case AVMEDIA_TYPE_VIDEO:
            ...
            ALOGI("copy_codec_parameters AVMEDIA_TYPE_VIDEO width(%d) height(%d) ", dst->width, dst->height);
            break;
        case AVMEDIA_TYPE_AUDIO:
            ...
            dst->block_align     = src->block_align;
            dst->frame_size      = src->frame_size;
            dst->delay           =
            dst->initial_padding = src->initial_padding;
            dst->seek_preroll    = src->seek_preroll;
            break;
