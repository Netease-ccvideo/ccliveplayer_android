...
#define FFP_BUFFERING_COUNT_PERIOD (5 * 60)
#define FFP_BUFFERING_COUNT_WARN_THRESHOLD 5
#define FFP_BUFFERING_COUNT_PERIOD_NOLIVE (1 * 60)
#define FFP_BUFFERING_COUNT_WARN_THRESHOLD_NOLIVE 5
#define FFP_BUFFERING_TAKE_WARN_THRESHOLD_NOLIVE 6000
...
static int64_t packet_queue_get_duration(PacketQueue *q) {
    int64_t duration = 0;
 //   SDL_LockMutex(q->mutex); // lock on android??
    if (q->duration > 0) {
        duration = q->duration;
    } else {
        if (q->first_pkt && q->last_pkt && q->first_pkt != q->last_pkt) {
    ...
    for (;;) {
        ...
        if (pkt1) {
            ...
            *pkt = pkt1->pkt;
...
#endif
            ret = 1;
            if(!is_audio_queue && ffp->radical_realtime_level == 3) {
            ...
            SDL_CondWaitTimeout(q->cond, q->mutex, 20);
            if (is_audio_queue && ffp->is->av_sync_type == AV_SYNC_AUDIO_MASTER) {
            ...
            if(!is_audio_queue && ffp->is->av_sync_type == AV_SYNC_VIDEO_MASTER && ffp->radical_realtime_level == 3) {
                if (!ffp->eof && frame_queue_nb_remaining(&ffp->is->pictq) <= 0) {
                    if(av_gettime_relative() - ffp->is->last_buffer_time > 2 * 1000 * 1000) {
                        ffp_notify_msg2(ffp, FFP_MSG_BUFFERING_ZHUAWAWA, 0);
                        ffp->is->last_buffer_time = av_gettime_relative();
                    }
                }
            }
        }
...
static int packet_queue_video_cleanup(FFPlayer *ffp, PacketQueue *q, const int64_t buffer_ts, int64_t* pts)
{
    ...
    *pts = 0;
    ...
    if (q->nb_packets > 0 && buffer_ts > 0 && duration > buffer_ts) {
        buffer_packet_count = (int)(buffer_ts / ((float)duration / q->nb_packets)) + 1;
        max_buffer_packet_count = buffer_packet_count * 3 / 2;

        if (q->nb_packets > max_buffer_packet_count) {
    ...
    do {
        ...
        switch (d->avctx->codec_type) {
            case AVMEDIA_TYPE_VIDEO: {
                ...
                if (got_frame) {
#ifdef __APPLE__
                    if(ffp->show_debug_info)
                        ffp->is->stat_info.vdps = SDL_SpeedSamplerAdd(&ffp->vdps_sampler, FFP_SHOW_VDPS_AVCODEC, "vdps[avcodec]");
#endif
...
static void decoder_destroy(FFPlayer* ffp, Decoder *d) {
    av_packet_unref(&d->pkt);
    if(ffp->codecIsNewAlloc)
...
void frame_queue_unref_items(FrameQueue *f)
{
    int i;
    for (i = 0; i < f->max_size; i++) {
        Frame *vp = &f->queue[i];
...
static void frame_queue_destroy_frames(FrameQueue *f)
{
    int i;
    for (i = 0; i < f->max_size; i++) {
        Frame *vp = &f->queue[i];
...
//            if(pic->format == SDL_FCC_I420)
//                displayFrame.format =  FRAME_FORMAT_YUV;
//            else if(pic->format == SDL_FCC_RV32)
...
//        }
        //ALOGI("stream_open--->before SDL_VoutDisplayYUVOverlay = %lld ms\n", (av_gettime_relative() - ffp->is->stream_open_time) / 1000);
        if (!ffp->display_ready) {
            ...
            if (!ffp->first_video_frame_rendered) {
                ...
                ALOGF("[open] render start %f s\n", (av_gettime_relative() - ffp->is->stream_open_time) / 1000.0f / 1000.0f);
            }
        }
        int display_ret = SDL_VoutDisplayYUVOverlay(ffp->vout, vp->bmp);
#ifdef __APPLE__
        if(ffp->show_debug_info)
            is->stat_info.vfps = SDL_SpeedSamplerAdd(&ffp->vfps_sampler, FFP_SHOW_VFPS_FFPLAY, "vfps[ffplay]");
#endif
        //ALOGI("stream_open--->SDL_VoutDisplayYUVOverlay = %lld ms\n", (av_gettime_relative() - ffp->is->stream_open_time) / 1000);
#ifdef FFP_SHOW_FPS
        int64_t dur = SDL_GetTickHR() - start;
...
static void video_no_image_display(FFPlayer *ffp)
{
    ...
    if (is->video_st) {
        ...
        if (lastvp->bmp && (lastvp->bmp->pixels == NULL || lastvp->bmp->pixels[0] != NULL)) {
            SDL_VoutDisplayYUVOverlay(ffp->vout, lastvp->bmp);
        }
    }
}

void stream_component_close(FFPlayer *ffp, int stream_index)
{
    VideoState *is = ffp->is;
    ...
    switch (avctx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            ALOGW("begin wait for audio_tid\n");
            decoder_abort(&is->auddec, &is->sampq);
            ALOGW("end wait for audio_tid\n");
            SDL_AoutCloseAudio(ffp->aout);

            decoder_destroy(ffp, &is->auddec);
            swr_free(&is->swr_ctx);
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            decoder_destroy(ffp, &is->viddec);

            break;
        case AVMEDIA_TYPE_SUBTITLE:
            ALOGW("begin wait for subtitle_tid\n");
            decoder_abort(&is->subdec, &is->subpq);
            ALOGW("end wait for subtitle_tid\n");
            decoder_destroy(ffp, &is->subdec);
            break;
        default:
            break;
    }
...
static void stream_close(FFPlayer *ffp)
{
    ...
    /* free all pictures */
    if (!ffp->vout->decode_out_raw_data) {
        frame_queue_destroy(&is->pictq);
    } else {
        frame_queue_destroy_frames(&is->pictq);
    }
...
static double compute_target_delay(double delay, VideoState *is, bool is_slow_playback_rate)
{
    ...
    if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER) {
        ...
        if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD) {
            ...
            else if ((diff >= sync_threshold && delay > AV_SYNC_FRAMEDUP_THRESHOLD) ||(diff >= sync_threshold && is_slow_playback_rate))
                delay = delay + diff;
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if ((now - is->last_refresh_time) / 1000 > DEFAULT_VIDEO_REFRESH_INTERVAL && !ffp->display_disable) {
            ...
            bool is_slow_playback_rate = ffp->pf_playback_rate < 0.5f;
            /* compute nominal last_duration */
            last_duration = vp_duration(is, lastvp, vp);
            if (redisplay)
                delay = 0.0;
            else
                delay = compute_target_delay(last_duration, is, is_slow_playback_rate);
            ...
            if (is->subtitle_st) {
                while (frame_queue_nb_remaining(&is->subpq) > 0) {
                    ...
                    {
                        if (sp->uploaded && ffp->subtitle_display)
...
static void alloc_picture(FFPlayer *ffp, int frame_format)
{
    ...
    vp->bmp = SDL_Vout_CreateOverlay(vp->width, vp->height,
                                   frame_format,
                                   ffp->vout, ffp->crop,
                                   ffp->surface_width, ffp->surface_height);
#ifdef FFP_MERGE
...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
    ...
    if(accurate_video_seek_if_need(ffp, pts) == 1) {
        return 1;
    }
    ...
    int frame_width = src_frame->width;
    int frame_height = src_frame->height;
    if(!ffp->vout->decode_out_raw_data && (ffp->rotate_degree == 90 || ffp->rotate_degree == 270))
    {
        frame_width = src_frame->height;
        frame_height = src_frame->width;
    }
    /* alloc or resize hardware picture buffer */
    if (!vp->bmp || vp->reallocate || !vp->allocated ||
        vp->width  != frame_width ||
        vp->height != frame_height || vp->frame_format != src_frame->format || vp->crop != ffp->crop) {
        if ((vp->width > 0 && vp->width != frame_width) || (vp->height > 0 && vp->height != frame_height)) {
            AVCodecContext *avctx = is->video_st->codec;
            ffp->rotate_degree = ffp_get_video_rotate_degrees(ffp);
            int width  = ffp->rotate_degree == 90 || ffp->rotate_degree == 270 ? avctx->height : avctx->width;
            int height = ffp->rotate_degree == 90 || ffp->rotate_degree == 270 ? avctx->width  : avctx->height;
    ...
    if (vp->bmp) {
        ...
        vp->bmp->rotate = ffp->rotate_degree;
            ...
            if ((audio_clock * 1000 * 1000 < is->seek_pos ) || deviation > MAX_DEVIATION) {
                ...
                if (audio_seek_pos == is->seek_pos) {
                    ...
                    if (audio_seek_pos == is->seek_pos && is->video_accurate_seek_req && !is->abort_request) {
                        ...
                        ALOGF("accurate_seek complete pts %d from audio\n",(int)(audio_clock * 1000));
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ...
            //ALOGD("%p video pts %lld dpts %f s\n",ffp, frame->pts, pts);

            ret = queue_picture(ffp, frame, pts, duration, av_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
//            if(ffp->enable_report_capture)
            //save_bak_video_frame(ffp, frame);
            av_frame_unref(frame);
#if CONFIG_AVFILTER
        }
#endif
...
int video_thread(void *arg)
{
    ...
    if(ret && !ffp->is->abort_request) {
        ALOGD("hardware decode failed use normal decoder \n");
        ffpipenode_free_p(&ffp->node_vdec);
        ffp->node_vdec = NULL;
        ffp->videotoolbox = 0;
        ffp->pipeline->opaque->mediacodec_enabled = 0;
...
//            }
        ...
        if (!isnan(af->pts)) {
            double pts_interval = (double) af->frame->nb_samples / af->frame->sample_rate;
            is->stat_info.audio_pts_interval = pts_interval;
            is->audio_clock = af->pts + pts_interval;
        }
        else
            is->audio_clock = NAN;
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
   ...
   if (ffp->pf_playback_rate_changed) {
    ...
    if (!isnan(is->audio_clock)) {
        ...
        updateBufferStaticsTime(ffp);

        set_clock_at(&is->audclk, dPts - SDL_AoutGetLatencySeconds(ffp->aout), is->audio_clock_serial, ffp->audio_callback_time / 1000000.0);
...
int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    FFPlayer *ffp = opaque;
    ...
    while (SDL_AoutOpenAudio(ffp->aout, &wanted_spec, &spec) < 0) {
        if (ffp->is->abort_request) {
            ALOGI("SDL_AoutOpenAudio ing --> abort_request \n");
            return -1;
        }
        if(!max_audio_open_times--) {
            ALOGI("SDL_AoutOpenAudio failed return \n");
            return -1;
        }
...
//    return is->abort_request;
    int64_t max_open_input_time = is->max_open_input_time > 0 ? is->max_open_input_time :  10 * 1000 * 1000;
    return is->abort_request || (!is->prepared && (is->open_input_time > 0 && av_gettime_relative() - is->open_input_time >= max_open_input_time));
}
...
void init_jitter_calculator(JitterCalculator *jc, int period_ms) {
    printf("%s period=%d\n", __func__, period_ms);
    jc->max_jitter = 0;
...
open_retry:
    ...
    if (err < 0) {
        if (open_input_retry_num >= max_open_input_stream_retry_num || decode_interrupt_cb(is)) {
            ALOGF("[open] open input error %d:%s max retry(%d) reached\n", err, av_err2str(err), open_input_retry_num);
            print_error(is->filename, err);
            last_error = err;
            ret = -1;
            goto fail;
        } else {
            ALOGF("[open] open input error %d:%s\n", err, av_err2str(err));
...
//#endif
    ...
    if (err < 0) {
        if (open_input_retry_num >= max_open_input_stream_retry_num || decode_interrupt_cb(is)) {
            ALOGF("[open] find stream info error %d:%s max retry reached\n", err, av_err2str(err));
            ...
            ALOGF("[open] find stream info error %d:%s\n", err, av_err2str(err));
            int open_input_retry_delay = primary_open_input_retry_delay + open_input_retry_delay_advance * open_input_retry_num;
            int sleeped = 0;
            while (!is->abort_request && sleeped < open_input_retry_delay) {
                SDL_Delay(50);
...
#ifdef __ANDROID__
    if (ffp && !ffp->realtime_play)
        is->max_frame_duration = 5; // 处理点播拖动视频可能卡画面问题
#endif
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            video_stream_count++;
            if (codec->codec_id == AV_CODEC_ID_H264 || codec->codec_id == AV_CODEC_ID_HEVC) {
                h264_stream_count++;
    ...
    if (!ffp->audio_disable) {
        if (ffp->audioLanguage) {
            for (i = 0; i < ic->nb_streams; i++) {
                AVStream *st = ic->streams[i];
                enum AVMediaType type = st->codec->codec_type;
                AVDictionaryEntry *avdict = NULL;
                AVCodecContext *codec = ic->streams[i]->codec;
                if (codec->codec_type == AVMEDIA_TYPE_AUDIO){
                    avdict = av_dict_get(st->metadata, "language", NULL, AV_DICT_MATCH_CASE);
                    if(!avdict)
                        continue;
                    if (strcmp(avdict->value, ffp->audioLanguage) == 0)
    ...
    if (!ffp->video_disable && ffp->enable_subtitle) {
        if (ffp->subtitleLanguage) {
            for (i = 0; i < ic->nb_streams; i++) {
                AVStream *st = ic->streams[i];
                enum AVMediaType type = st->codec->codec_type;
                AVDictionaryEntry *avdict = NULL;
                ...
                if (codec->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                    ...
                    if(!avdict)
                        continue;
                    if (strcmp(avdict->value, ffp->subtitleLanguage) == 0)
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ...
        is->stat_info.video_bitrate = (int)vcodec->bit_rate;
    ...
    int64_t prev_tick_counter = av_gettime_relative();

    init_jitter_calculator(&is->jitter_calculator, 5 * 1000);

    /* offset should be seeked*/
    if (ffp->seek_at_start > 0 && !ffp->realtime_play) {
        ffp_seek_to_l(ffp, (long)ffp->seek_at_start);
    }

    /* init saver */
    if (ffp->saver) {
        ffp->saver->videosave_index = st_index[AVMEDIA_TYPE_VIDEO];
        ffp->saver->audiosave_index = st_index[AVMEDIA_TYPE_AUDIO];
        if(ffp_init_save(ffp) != 0){
            ffp_stop_save(ffp, SAVE_ERROR_FOR_INIT_CACHE);
        }
    }
...
//      of the seek_pos/seek_rel variables
        ...
        if (is->pause_keep_req) {
            ALOGI(" pause_keep_req \n");
            if(is->pause_keep_ms > 0) {
                ALOGI(" pause_keep_req exe %f \n", is->pause_keep_ms);
                ffp_clean_pkt_buffer(ffp, is->pause_keep_ms);
                is->pause_keep_ms = 0;
            }
            is->pause_keep_req = 0;
        }
                ...
                && (is->subtitleq.nb_packets > MIN_FRAMES || is->subtitle_stream < 0 || is->subtitleq.abort_request))))
        {
            if (ffp->is->buffering_on && is->audioq.size + is->videoq.size > ffp->max_buffer_size) {
                ALOGW("too much data: audio packet count=%d size=%d, video packet count=%d size=%d \n", is->audioq.nb_packets, is->audioq.size, is->videoq.nb_packets, is->videoq.size);
                int64_t pts = 0;
                if (packet_queue_video_cleanup(ffp, &is->videoq, ffp->buffering_target_duration_ms, &pts)) {
                    ...
                    ALOGW("clean down data: audio packet count=%d size=%d, video packet count=%d size=%d \n", is->audioq.nb_packets, is->audioq.size, is->videoq.nb_packets, is->videoq.size);
        ...
        if(!is->audio_st) {
            video_empty = (is->viddec.finished == is->videoq.serial && frame_queue_nb_remaining(&is->pictq) == 0);
        } else {
            video_empty = (is->viddec.finished == is->videoq.serial || frame_queue_nb_remaining(&is->pictq) == 0);
        }

        feof_sleep_ms = ffp->loop != 1 && video_empty && ffp->eof ? 10 : 100;

        if ((!is->paused || completed) &&
            (!is->audio_st || (is->auddec.finished == is->audioq.serial && frame_queue_nb_remaining(&is->sampq) == 0)) &&
            (!is->video_st || ((ffp->display_disable && ffp->loop != 0)|| video_empty))) {
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {//有可能在弱网 情况下异常，无法发送结束消息
        ...
        if (!ffp->eof) {
            ...
            if (ret < 0) {
                ...
                //可以时，写入文件
                if (ffp->saver) {
                    int ret = ffp_save_file(ffp, pkt);
                    if (ret != 0) {
                        ffp_stop_save(ffp, SAVE_ERROR_FOR_CACHE_FILE);
                    }
                }
                ffp->eof = 0;
            }
            ...
            if (is->buffering_on) {
                ...
                if (percent != pre_percent) {
                    if (ffp->display_ready && percent > pre_percent) {
                        ...
                        if( !ffp->realtime_play && is->stat_info.buffer_begin_time != 0) {
                            long buffering_end_time = (long)(av_gettime_relative() / 1000);
                            long buffering_take = buffering_end_time - is->stat_info.buffer_begin_time;
                            if(buffering_take >= FFP_BUFFERING_TAKE_WARN_THRESHOLD_NOLIVE && !is->stat_info.buffer_warn_flag)
                            {
                                ALOGI("buffer on start:%ld end:%ld diff:%ld\n", is->stat_info.buffer_begin_time, buffering_end_time, buffering_take);
                                ffp_notify_msg2(ffp, FFP_MSG_BUFFERING_TAKE_WARNING, (int)buffering_take);
                                is->stat_info.buffer_warn_flag = 1;
                                //is->stat_info.buffer_count_pre = is->stat_info.buffer_count;
                            }
                        }
                    ...
                    if (percent >= 100) {
                        ...
                        if (!is->first_buffering_ready) {
                            ...
                            ALOGF("[open] stream_open--->first buffering time %lld ms\n", (av_gettime_relative() - ffp->is->stream_open_time) / 1000);
                        }
                        ffp_toggle_buffering(ffp, 0);
...
//                }
                ...
                {
                    ffp_notify_msg1(ffp, FFP_MSG_GOT_FIRST_AUDIO_FRAME);
                    first_audio_frame = 0;
                    is->force_videodisplay = 0;//一直没有音频呢？？

                }

                audio_packet_count++;
                if(is->av_sync_type != AV_SYNC_AUDIO_MASTER && ffp->radical_realtime_level != RadicalLevel_3 )
                {
...
//                printf("[video packet] %lld duration=%lld avg=%lld\n", video_packet_count, packet_queue_get_duration(&is->videoq), packet_queue_get_duration(&is->videoq) / is->videoq.nb_packets);
                ...
                {
                    ffp_notify_msg1(ffp, FFP_MSG_GOT_FIRST_VIDEO_FRAME);
                    if (first_audio_frame && ffp->realtime_play) {
				...
				//check jitter
                if (ffp->realtime_play) {
                    if (ffp->radical_realtime_level <= 0) {   // 非激进缓冲
                        ...
                        if (ffp->realtime_video_packet_count % FORWARD_CHECK_PACKET_COUNT == 0
                            ...
                            bool turnDown = adjust_buffering_target_duration(ffp, false, false);
                            if (turnDown) {
                        ...
                        int min_video_buffer = 200 / ffp->radical_realtime_level;
                        int max_video_buffer = 500 / ffp->radical_realtime_level;
                        if(min_video_buffer < 50)
                            min_video_buffer = 50;
                         if (max_video_buffer < 50)
                             max_video_buffer = 50;
                        if (forward > 0) {
                            int64_t cur_video_duration = packet_queue_get_duration(&is->videoq);
                            ...
                            ALOGD("forward=%d, buffering_target_duration_ms_limit=%d, cur_video_duration=%lld\n", forward, ffp->buffering_target_duration_ms_limit, cur_video_duration);
                        }
                        int64_t pts = 0;
            ...
            if (!is->buffering_on && ffp->realtime_play
                && ffp->buffering_target_duration_ms < ffp->buffering_target_duration_ms_min
                && abs((int)(av_gettime_relative() - prev_tick_counter)) > 500) {
                        prev_tick_counter = av_gettime_relative();
                        adjust_buffering_target_duration(ffp, true, true);
            }
        } else {
            if(ffp->saver)
                ffp_stop_save(ffp, SAVE_SUCESS);

            if (is->buffering_on) {
                ffp_toggle_buffering(ffp, 0);
            }
            SDL_Delay(feof_sleep_ms);  // 循环播放的时候，需要马上seek到0,所以减少sleep时长
            SDL_LockMutex(wait_mutex);
            SDL_CondWaitTimeout(is->continue_read_thread, wait_mutex, 10);
            SDL_UnlockMutex(wait_mutex);
        }
        if(ffp->eof && is->paused)
        {
            SDL_Delay(10);
        }
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    ffp->connwait = (is->stream_open_time - ffp->created_tick) / 1000000.0f;
    ...
    is->pause_keep_ms = 0.0f;
    is->stat_info.last_buffer_warn_time = 0;
    is->stat_info.buffer_warn_flag = 0;
    is->stat_info.playable_time = 0;
    is->stat_info.playable_time_sum = 0;
    is->stat_info.audio_pts_interval = 0;
    is->read_tid = NULL;
    is->video_tid = NULL;
    is->audio_tid = NULL;
    is->audio_stream = -1;
    is->video_stream = -1;
    is->force_videodisplay_st = 0;
    ...
    if (frame_queue_init(&is->subpq, &is->subtitleq, SUBPICTURE_QUEUE_SIZE, 0) < 0)
        goto fail;
    if (frame_queue_init(&is->sampq, &is->audioq, ffp->sampq_size, 1) < 0)
        goto fail;
    ...
    is->max_open_input_time = ffp->max_open_input_time;
    is->play_mutex = SDL_CreateMutex();
    is->accurate_seek_mutex = SDL_CreateMutex();
    ffp->is = is;

    if (strstr(filename, "udp.v.cc.163.pull.com")) {
        ALOGI("! wow it's udp stream ! \n");
    }
    ...
    if(ffp->radical_realtime_level == RadicalLevel_3) {
        ffp->is->av_sync_type = AV_SYNC_VIDEO_MASTER;
    }
...
static int video_refresh_thread(void *arg)
{
    ...
    while (!is->abort_request) {
        ...
        {
            // do SDL_VoutDisplayInit() here to accelarate first video frame
            SDL_VoutDisplayInit(ffp->vout);
...
int capture_callback(void *opaque, void *snap_data, int format, int pitch, int width, int height)
{
  //  if (s_capture_callback)
  //      return s_capture_callback(opaque,snap_data, format, pitch, width, height);
    return 0;
}
...
FFPlayer *ffp_create(int crop)
{
    ...
    ffp->is = NULL;
    ffp->created_tick = av_gettime_relative();
    return ffp;
}
...
void ffp_destroy(FFPlayer *ffp)
{
    ...
    av_freep(&ffp->audioLanguage);
    av_freep(&ffp->subtitleLanguage);
    ...
    // message线程结束后才能destroy msg_queue，以下代码放在该函数外执行(android 改动)
#ifdef __APPLE__
    msg_queue_destroy(&ffp->msg_queue);
    av_free(ffp);
    ALOGF("ffp_destroy done %p \n",ffp);
#endif
}

void ffp_destroy_p(FFPlayer **pffp)
{
    ALOGF("ffp_destroy_p %p\n", *pffp);
    if (!pffp)
        return;
...
void ffp_set_format_callback(FFPlayer *ffp,  void *opaque)
{
...
void ffp_set_sample_queue_capicity(FFPlayer *ffp, int sample_count)
{
    ffp->sampq_size = sample_count;
}
...
void ffp_set_start_seek_pos(FFPlayer *ffp, int seekpos)
{
    ffp->seek_at_start = seekpos;
}

void ffp_set_videotoolbox(FFPlayer *ffp, int videotoolbox)
{
    ffp->videotoolbox = videotoolbox;
}

void ffp_set_loop_number(FFPlayer *ffp, int loop_number)
{
    ffp->loop = loop_number;
}

void ffp_set_disable_video(FFPlayer* ffp,int video_disable)
{
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name)
{
 ...
 //   ffp_show_version_str(ffp, "FFmpeg",         av_version_info());
    ...
    ffp_show_dict(ffp, "sws-opts   ", ffp->sws_opts);
    ...
    if (av_stristart(file_name, "rtmp", NULL) ||
        ...
        av_log(ffp, AV_LOG_INFO, "remove 'timeout' option for rtmp.\n");
    ...
    ALOGF("ffp_prepare_async_l done ffp->is = is %p \n",ffp);
    return 0;
}
...
int ffp_stop_l(FFPlayer *ffp)
{
    ...
    VideoState *is = ffp->is;
    if (ffp->saver)
        ffp_stop_save(ffp, SAVE_ERROR_FOR_UNFINISH);
    if (is) {
        is->abort_request = 1;
        toggle_pause(ffp, 1);
    }
...
int ffp_wait_stop_l(FFPlayer *ffp)
{
    ...
    if (ffp->is) {
        ...
        stream_close(ffp);
        ffp->is = NULL;
        ALOGF("ffp->is = NULL ffp_wait_stop_l %p \n",ffp);
    }
    ALOGF("ffp_wait_stop_l done %p \n",ffp);
    return 0;
}
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    if (ffp->saver) {
        ffp_stop_save(ffp, SAVE_ERROR_FOR_SEEK);
    }
    return 0;
}
...
void ffp_queue_bak_vtb_picture(FFPlayer* ffp, AVFrame *src_frame)
{
   // save_bak_video_frame(ffp, src_frame);
}
...
void ffp_toggle_buffering_l(FFPlayer *ffp, int buffering_on)
{
    ...
    if (buffering_on && !is->buffering_on) {
        if (ffp->radical_realtime_level <= 0)
            adjust_buffering_target_duration(ffp, true, false);
        ALOGD("player buffering start, target(%d), current:audio(%lld) sample(%lld) video(%d) picture(%d)\n", ffp->buffering_target_duration_ms, packet_queue_get_duration(&is->audioq), packet_queue_get_duration(&is->videoq), is->sampq.size, is->pictq.size);
        is->buffering_on = 1;
        ...
        if (ffp->display_ready) {
            if(!ffp->realtime_play) {
                is->stat_info.buffer_begin_time = (long)(av_gettime_relative() / 1000);
            }
            if (is->seek_req) {
                is->seek_buffering = 1;
        ...
        if (ffp->display_ready) {
            if(!ffp->realtime_play) {
                is->stat_info.buffer_begin_time = 0;
            }
...
void ffp_set_subtitle_codec_info(FFPlayer *ffp, const char *module, const char *codec)
{
    ...
    ALOGI("SubtitleCodec: %s", ffp->audio_codec_info);
}

void ffp_set_language(FFPlayer *ffp, const char *audioLanguage, const char *subtitleLanguage)
{
    av_freep(&ffp->audioLanguage);
    av_freep(&ffp->subtitleLanguage);
    ffp->audioLanguage = av_strdup(audioLanguage);
    ffp->subtitleLanguage = av_strdup(subtitleLanguage);
}

void ffp_set_sub_display(FFPlayer *ffp, bool enable)
{
    ffp->subtitle_display = enable;
}

void ffp_set_save_to_local(FFPlayer *ffp, const char *path)
{
    FileSaver *saver = av_mallocz(sizeof(FileSaver));
    saver->save_path = av_strdup(path);
    ffp->saver = saver;
}
...
int ffp_get_video_rotate_degrees(FFPlayer *ffp)
{
    ...
    double ro = get_rotation(is->video_st);
    ALOGI("[rotation] ro(%lf)", ro);
    int64_t i64ro = (int64_t)round(fabs(ro));
    int iro = (int)(i64ro % 360);
    int theta  = abs(iro);
    ALOGI("[rotation] ro(%lf) i64ro(%lld) iro(%d) theta(%d)", ro, i64ro, iro, theta);
...
float ffp_get_property_float(FFPlayer *ffp, int id, float default_value)
{
    switch (id) {
        ...
        case FFP_PROP_FLOAT_VIDEO_FPS:
            return ffp ? ffp->is->stat_info.video_framerate : default_value;
        default:
...
bool adjust_buffering_target_duration(FFPlayer *ffp, bool turnUp, bool force) {

    ALOGI("[buffer] -- adjust_buffering_target_duration -- turnIp(%d) force(%d)\n", turnUp, force);
    ...
    if (turnUp) {
        ...
        if (dist_sec > 0) {
            ...
            if (old < ffp->buffering_target_duration_ms_min && target > ffp->buffering_target_duration_ms_min)
                target = ffp->buffering_target_duration_ms_min;
    ...
    if (ffp->buffering_target_duration_ms < ffp->buffering_target_duration_ms_min) {
        if (old > ffp->buffering_target_duration_ms_min)
            ...
            if (ffp->buffering_target_duration_ms < ffp->buffering_target_duration_ms_first)
                ffp->buffering_target_duration_ms = ffp->buffering_target_duration_ms_first;
        }
    }
    ...
    ALOGI("[buffer] change buffer old(%llu) new(%d) turnUp(%d) force(%d)", old, ffp->buffering_target_duration_ms, turnUp, force);
...
void ffp_set_crop_mode(FFPlayer *ffp, bool crop, int surface_width, int surface_height)
{
    ...
    if (is) {
        ...
        for (int i = 0; i < is->pictq.max_size; i++) {
            SDL_VoutOverlay *overlay = is->pictq.queue[i].bmp;
            if (overlay) {
                overlay->crop = crop;
...
void ffp_set_scaled_mode(FFPlayer *ffp, int mode, bool transparentBackground)
{
...
static int ffp_capture_frame_func(void *arg) {
    FFPlayer *ffp = arg;
    if(ffp->is->abort_request) {
        ALOGI("ffp_capture_frame_func abort \n");
        return 0;
    }
    if (NULL != ffp && NULL != ffp->is && !ffp->is->frame_capturing && NULL != ffp->vout && NULL != ffp->vout->mutex) {
        ...
        Frame *vf = NULL;
        int max_capture_times = 3;
        while(max_capture_times) {
            vf = frame_queue_peek_readable(&ffp->is->pictq);
            if(vf) {
                break;
            }
            SDL_Delay(100);
        ...
        if(!vf) {
            ALOGI("thumbnailImage ffp_capture_frame_func no overlay\n");
            capture_callback(ffp->capture_opaque, NULL, 0, 0, 0, 0);
            ffp->is->frame_capturing = 0;
            SDL_UnlockMutex(ffp->vout->mutex);
            return 0;
        }
        SDL_VoutOverlay *overlay = vf->bmp;//ffp->is->pictq.queue[0].bmp;
        ALOGI("thumbnailImage overlay %p \n", overlay);
#else
        SDL_VoutOverlay *overlay = ffp->is->pictq.queue[0].bmp;
#endif
        SDL_VoutLockYUVOverlay(overlay);
        if (overlay) {
            ffp->is->frame_capturing = 1;
#ifdef __APPLE__
            if(overlay->format == SDL_FCC__VTB) {
                void* pixel_buffer = overlay->pixel_buffer;
                if(pixel_buffer != NULL) {
                    capture_callback(ffp->capture_opaque, pixel_buffer, overlay->format, 0, 0, 0);
                    ALOGI("thumbnailImage ffp_capture_frame_func get pixel_buffer\n");
                } else {
                    capture_callback(ffp->capture_opaque, NULL, 0, 0, 0, 0);
                     ALOGI("thumbnailImage ffp_capture_frame_func no pixel \n");
                }

            } else if(overlay->format == SDL_FCC_I420){
                uint8_t *argbData = NULL;
                int pitch = 0;
                int ret = SDL_VoutFFmpeg_ConverI420ToARGB(overlay, &ffp->is->img_convert_ctx, ffp->sws_flags, &argbData, &pitch);
                if(ret == 0) {
                    int width = overlay->w;
                    int height = overlay->h;
                    capture_callback(ffp->capture_opaque, argbData, SDL_FCC_I420, pitch, width, height);
                     ALOGI("thumbnailImage ffp_capture_frame_func get argb data \n");
                } else {
                    capture_callback(ffp->capture_opaque, NULL, 0, 0, 0, 0);
                    ALOGI("thumbnailImage ffp_capture_frame_func get argb data failed \n");
                }
            }
#else
            uint8_t *argbData = NULL;
            int pitch = 0;
            SDL_VoutFFmpeg_ConverI420ToARGB(overlay, &ffp->is->img_convert_ctx, ffp->sws_flags, &argbData, &pitch);
            int width = overlay->w;
            int height = overlay->h;
        ...
        ALOGD("capture 2");
        ffp->is->frame_capturing = 0;
        SDL_VoutUnlockYUVOverlay(overlay);
        SDL_UnlockMutex(ffp->vout->mutex);
    }
    return 0;
}
...
{
//    ffp->is->capture_thread = SDL_CreateThreadEx(&ffp->is->_capture_thread, ffp_capture_frame_func, ffp, "capture_thread");

}

void ffp_free_capture_frame_resource(FFPlayer *ffp)
{
    SDL_LockMutex(ffp->vout->mutex);
    VideoState *is = ffp->is;
    av_free(is->capture_frame_data);
...
/**
 * update buffer statics time
 */
void updateBufferStaticsTime(FFPlayer *ffp) {

    VideoState* is = ffp->is;
    int warn_period = ffp->realtime_play ? FFP_BUFFERING_COUNT_PERIOD : FFP_BUFFERING_COUNT_PERIOD_NOLIVE;
    long warn_time = (long)(av_gettime_relative() / 1000000);
    if(is->stat_info.last_buffer_warn_time == 0) {
        is->stat_info.last_buffer_warn_time = warn_time;
    }
    if(warn_time - is->stat_info.last_buffer_warn_time > warn_period ) {
        ALOGE("reset buffer statics time start(%zd) now(%ld) cnt(%d) \n ",is->stat_info.last_buffer_warn_time, warn_time, is->stat_info.buffer_count);
        is->stat_info.last_buffer_warn_time = warn_time;
        is->stat_info.buffer_warn_flag = 0;
       // is->stat_info.buffer_count_pre = is->stat_info.buffer_count;
    }
...
/**
 * check buffer count, if count bigger than max,than notify
 */
void checkBufferCnt(FFPlayer *ffp) {

    VideoState* is = ffp->is;
    int warn_threshold = ffp->realtime_play ?  FFP_BUFFERING_COUNT_WARN_THRESHOLD : FFP_BUFFERING_COUNT_WARN_THRESHOLD_NOLIVE;
/*    int buffer_cnt = is->stat_info.buffer_count - is->stat_info.buffer_count_pre;
    ALOGE("check buffering count total(%d) buffer_cnt_in_period(%d)\n ",is->stat_info.buffer_count, buffer_cnt);
    if(buffer_cnt >= warn_threshold && !is->stat_info.buffer_warn_flag) {
        ALOGE("buffering too frequent (%d) post msg to app \n ", buffer_cnt);
        ffp_notify_msg2(ffp, FFP_MSG_BUFFERING_COUNT_WARNING, buffer_cnt);
        is->stat_info.buffer_warn_flag = 1;
    }*/
}
...
void calc_download_byte_per_second(VideoState* is, int size) {

    static int sec_cnt = 0;              // 秒计数
    static int sum_download_per_min = 0; // 每秒下载数和，累积一分钟
    int64_t cur_time = av_gettime_relative();
    is->cur_recv_size += size;
    ...
    if (is->last_time / 1000000 != cur_time / 1000000)
    {
     ...
     if (sec_cnt > 60) {
         ...
         sec_cnt = 0;
         sum_download_per_min = 0;
         ALOGD("loadbytes stat---> download_per_min:%d\n", is->stat_info.download_per_min);
     }
     sum_download_per_min += is->stat_info.download_bps;
    }
}

 int handle_seek(void* handle, int* pcompleted, int *pRet) {

     FFPlayer* ffp = (FFPlayer*)handle;
     VideoState* is = ffp->is;
...
//      of the seek_pos/seek_rel variables
         ...
         *pRet = avformat_seek_file(is->ic, -1, seek_min, seek_target, seek_max, is->seek_flags);
         if (*pRet < 0) {
             av_log(NULL, AV_LOG_ERROR,
                    "%s: error while seeking\n", is->ic->filename);
         } else {
         ...
         *pcompleted = 0;
         ...
         ffp_notify_msg3(ffp, FFP_MSG_SEEK_COMPLETE, (int)fftime_to_milliseconds(seek_target), *pRet);
         ffp_toggle_buffering(ffp, 1);
     }
     return READ_RET_OK;
 }
 ...
 int handle_attachment_req(void* handle, int* pRet) {
     ...
     if (is->queue_attachments_req) {
         if (is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC)) {
             AVPacket copy;
             if ((*pRet = av_copy_packet(&copy, &is->video_st->attached_pic)) < 0)
 ...
 int handle_full_queue(void* handle, SDL_mutex *wait_mutex) {
                 ...
                 && (is->subtitleq.nb_packets > MIN_FRAMES || is->subtitle_stream < 0 || is->subtitleq.abort_request))))
     {
         if (ffp->is->buffering_on && is->audioq.size + is->videoq.size > ffp->max_buffer_size) {
             ...
             SDL_CondWaitTimeout(is->continue_read_thread, wait_mutex, 10);
             SDL_UnlockMutex(wait_mutex);
             return READ_RET_CONTINE;
         }
     }
     return READ_RET_OK;
 }

 void handle_read_frame_error(void* handle, int ret, SDL_mutex *wait_mutex) {
     ...
     if ((ret == AVERROR_EOF || avio_feof(ic->pb)) && !ffp->eof) {
         ALOGE("!!!EOF!!! \n");
         packet_queue_put_null_packet(is);
         if (is->subtitle_stream >= 0)
             packet_queue_put_nullpacket(&is->subtitleq, is->subtitle_stream);
         ffp->eof = 1;
     }
 ...
 void packet_queue_put_null_packet(void* video_state) {
 ...
 * 更新buffering状态
 */
 void handle_buffering(void* handle, int audio_stream_count, int64_t audio_packet_count, int* pPrePercent) {
     ...
     int target_buffer = ffp->buffering_target_duration_ms > 0 ? ffp->buffering_target_duration_ms : JITTER_DEFAULT;
     int percent = 0, pre_percent = *pPrePercent;
     if (audio_stream_count > 0) {
         percent = (packet_queue_get_duration(&is->audioq) / (float)target_buffer) * 100;
     } else {
         percent = (packet_queue_get_duration(&is->videoq) / (float)target_buffer) * 100;
     ...
     if (percent != pre_percent) {
         ...
         if (percent >= 100) {
             ...
             if (!is->first_buffering_ready) {
                 ...
                 ALOGW("stream_open--->first buffering time %.3f s\n", is->stat_info.first_buffering_time);
                 ffp_notify_msg1(ffp, FFP_MSG_FIRST_BUFFERING_READY);
                ALOGF("[open] stream_open--->first buffering ready %lld ms\n", (av_gettime_relative() - ffp->is->stream_open_time) / 1000);
             }
             ffp_toggle_buffering(ffp, 0);
             if (is->last_buffer_time != 0) {
                 ffp->is->stat_info.buffer_pre = (av_gettime_relative() - ffp->is->last_buffer_time) / 1000000.0;
...
//检查jitter
void check_jitter(FFPlayer *ffp, int64_t* plast_video_packet_recv_time, int64_t video_packet_recv_time) {

    VideoState *is = ffp->is;
    if (ffp->realtime_play) {
        int64_t last_video_packet_recv_time = *plast_video_packet_recv_time;
        if (!ffp->radical_realtime_level) {
            ffp->realtime_video_packet_count++;
            if (ffp->realtime_video_packet_count % FORWARD_CHECK_PACKET_COUNT == 0
                && !ffp->is->buffering_on
                && (ffp->is->last_stuck_time == 0 || (av_gettime_relative() - ffp->is->last_stuck_time) / 1000 > SAFE_INTERVAL_SINCE_STUCK)){
                bool turnDown = adjust_buffering_target_duration(ffp, false, false);
                if (turnDown) {
                    cleanup_packet_queue_by_buffer_time(ffp, ffp->buffering_target_duration_ms);
                }
            }
        } else {
            update_radical_buffer_duration(ffp, last_video_packet_recv_time, video_packet_recv_time);
            *plast_video_packet_recv_time = video_packet_recv_time;
            if (ffp->buffering_target_duration_ms_limit > 0) {
                if (cleanup_packet_queue_by_buffer_time(ffp, ffp->buffering_target_duration_ms_limit))
                    ffp->buffering_target_duration_ms_limit = 0;
            }
        }
    }
 ...
 bool cleanup_packet_queue_by_buffer_time(void *handle, int duration) {

     FFPlayer* ffp = (FFPlayer*) handle;
     VideoState* is = ffp->is;
     int64_t pts = 0;
     bool ret = false;
     if (packet_queue_video_cleanup(ffp, &is->videoq, duration, &pts)) {
         if (is && is->audio_st && is->video_st) {
             pts = av_rescale_q(pts, is->video_st->time_base, is->audio_st->time_base);
         }
 ...
 void update_radical_buffer_duration(void* handle, int64_t last_video_packet_recv_time, int64_t video_packet_recv_time) {
    ALOGI("update_radical_buffer_duration");
     FFPlayer* ffp = (FFPlayer*)handle;
     VideoState* is = ffp->is;
     int video_jitter = 0;
     int forward = 0;
     ...
     if (forward > 0) {
         ...
         ALOGI("forward=%d, buffering_target_duration_ms_limit=%d, cur_video_duration=%d\n", forward, ffp->buffering_target_duration_ms_limit, cur_video_duration);
     }
 }

 bool is_pkt_in_play_range(void* handle, bool is_video, int64_t pts) {
     ...
     AVRational stream_time_base = AV_TIME_BASE_Q;
     if(is_video) {
         if(is->video_st) {
             stream_start_time = is->video_st->start_time;
             stream_time_base = is->video_st->time_base;
         }
     } else {
         if(is->audio_st) {
             stream_start_time = is->audio_st->start_time;
             stream_time_base =  is->audio_st->time_base;
         }
     }
     bool pkt_in_play_range = ffp->duration == AV_NOPTS_VALUE ||
                              (pts - (stream_start_time != AV_NOPTS_VALUE ? stream_start_time : 0)) *
                              av_q2d(stream_time_base) -
                              (double)(ffp->start_time != AV_NOPTS_VALUE ? ffp->start_time : 0) / 1000000
                              <= ((double)ffp->duration / 1000000);
...
void ffp_set_bak_section(FFPlayer* ffp, long start, long end) {

    if(!ffp || !ffp->report_info)
        return;
    ffp->report_info->section_start = start;
    ffp->report_info->section_end = end;
}

void ffp_enter_background_foreground(FFPlayer* ffp, bool background, bool pause_in_background) {

    if(!pause_in_background || !ffp) return;
    VideoState *is = ffp->is;
    if(background) {
        is->enter_background_tick = av_gettime_relative();
    } else {
		if(is->enter_background_tick == 0) return;
        int64_t time = av_gettime_relative() - is->enter_background_tick;
        float ms = time / 1000.0f;
        is->pause_keep_req = 1;
        is->pause_keep_ms = ms;
        ALOGI("ffp_clean_buffer pause - resume take %f", ms);
    }
}

void ffp_clean_pkt_buffer(FFPlayer* ffp, float ms) {

    if(!ffp || !ffp->is) return;

    VideoState *is = ffp->is;
    PacketQueue *q = &is->videoq;
    ...
    float duration_s = duration * av_q2d(is->video_st->time_base) * 1000;
    ALOGI("ffp_clean_buffer duration %f %f\n",duration_s, ms);

    int buffer_packet_count = 0;
    if(q->nb_packets > 0) {
        float pkt_dur = (float)duration_s / q->nb_packets;
        if(pkt_dur > 0) {
            buffer_packet_count = (int)(ms / pkt_dur);
        }
    }
    if(buffer_packet_count == 0)
    {
        SDL_UnlockMutex(q->mutex);
        return;
    }
    int64_t pts = 0;
    item = q->first_pkt;
    if (q->nb_packets > 0 && ms > 0) {
        item = q->first_pkt;
        int key_interval = 0;
        while(item) {
            if (item->pkt.flags & AV_PKT_FLAG_KEY) {
                last_key_frame_index = index;
                ALOGI("ffp_clean_buffer last_key_frame_index %d, nb %d\n",last_key_frame_index, q->nb_packets);
            }
            index++;
            item = item->next;
            if (index >= buffer_packet_count) {
                break;
            }
            if(q->abort_request)
                break;
        ...
        if(last_key_frame_index > 0 && pts != AV_NOPTS_VALUE) {
            packet_queue_audio_cleanup(ffp, &is->audioq, pts);
        }
...
void ffp_get_test_info(FFPlayer *ffp, char* msg, int len) {
