...
typedef struct SDL_AMediaCodec_fields_t {
    ...
    jmethodID jmid_getOutputBuffer;
...
int SDL_AMediaCodecJava__loadClass(JNIEnv *env)
{
    ...
    if(sdk_int > IJK_API_20_KITKAT_WATCH) {
        IJK_FIND_JAVA_METHOD(env, g_clazz.jmid_getOutputBuffer, g_clazz.clazz,
...
static uint8_t* SDL_AMediaCodecJava_getOutputBuffer(SDL_AMediaCodec* acodec, size_t idx, size_t *out_size)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    jobject android_media_codec = opaque->android_media_codec;
    jobject local_output_buffer = (*env)->CallObjectMethod(env, android_media_codec, g_clazz.jmid_getOutputBuffer,idx);
    ...
    jlong size = (*env)->GetDirectBufferCapacity(env, local_output_buffer);
    void *ptr  = (*env)->GetDirectBufferAddress(env, local_output_buffer);
    SDL_JNI_DeleteLocalRefP(env, &local_output_buffer);
    if (out_size)
        *out_size = size;
    return ptr;
...
jobjectArray  SDL_AMediaCodecJava_getInputBuffers(JNIEnv *env, jobject android_media_codec){



    jobjectArray input_buffer_array = (*env)->CallObjectMethod(env, android_media_codec, g_clazz.jmid_getInputBuffers);
    if (SDL_JNI_CatchException(env) || !input_buffer_array) {
         ALOGE("%s: getInputBuffers failed\n", __func__);
         return NULL;
    }
    return input_buffer_array;
...
sdl_amedia_status_t SDL_AMediaCodecJava_queueInputBuffer(SDL_AMediaCodec* acodec, size_t idx, off_t offset, size_t size, uint64_t time, uint32_t flags)
{
    ...
    if (SDL_JNI_CatchException(env)) {
        return SDL_AMEDIA_ERROR_UNKNOWN;
        ALOGE("SDL_AMediaCodecJava_queueInputBuffer: failed");
    }
...
ssize_t SDL_AMediaCodec_writeInputData(SDL_AMediaCodec* acodec, size_t idx, const uint8_t *data, size_t size)
{
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    input_buffer_array = SDL_AMediaCodecJava_getInputBuffers(env, opaque->android_media_codec);
    ...
    if (SDL_JNI_CatchException(env) || !input_buffer) {
        ALOGE("%s: GetObjectArrayElement failed\n", __func__);
        goto fail;
    }
...
ssize_t SDL_AMediaCodecJava_dequeueOutputBuffer(SDL_AMediaCodec* acodec, SDL_AMediaCodecBufferInfo *info, int64_t timeoutUs, int rawenable)
{
    //ALOGI("%s(%d)", __func__, (int)timeoutUs);
...
//            SDL_JNI_DeleteGlobalRefP(env, &opaque->output_buffer_array);
            ...
            uint8_t* ptr = NULL;
            if(rawenable)
                ptr = SDL_AMediaCodec_getOutputBuffer(acodec, idx, &out_size);
            //ALOGI("%s: buffer ready (%d) ====================\n", __func__, idx);
            if (info) {
                ...
                info->bufferptr           = ptr;
                info->buffer_change_format_ptr     = NULL;
            }
        }
        break;
    }
...
sdl_amedia_status_t SDL_AMediaCodecJava_releaseOutputBuffer(SDL_AMediaCodec* acodec, size_t idx, bool render)
{
    ...
    if (SDL_JNI_CatchException(env)) {
        ALOGE("%s: releaseOutputBuffer index is %d, render is %d ", __func__, idx,render);
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
