...
static AudioChannelMapEntry g_audio_channel_map[] = {
    ...
    { 6, CHANNEL_OUT_STEREO, "6-chan", "CHANNEL_OUT_5POINT1" },//CHANNEL_OUT_5POINT1 would cause noisy audio with the phone.
};
...
typedef struct AudioTrack_fields_t {
    ...
    jmethodID getPlaybackRate;
    jmethodID setPlaybackRate;
...
static J4AC_android_media_PlaybackParams gclazz_PlaybackParams;


int SDL_Android_Playback_Params_global_init(JNIEnv *env) {

    int ret = -1;
    if (SDL_Android_GetApiLevel() < 23)
        goto ignore;


    jclass clazz;

    clazz = (*env)->FindClass(env, "android/media/PlaybackParams");
    IJK_CHECK_RET(clazz, -1, "missing PlaybackParams");

    // FindClass returns LocalReference
    gclazz_PlaybackParams.clazz = (*env)->NewGlobalRef(env, clazz);
    IJK_CHECK_RET(gclazz_PlaybackParams.clazz, -1, "AudioTrack NewGlobalRef failed");
    (*env)->DeleteLocalRef(env, clazz);

    gclazz_PlaybackParams.setSpeed = (*env)->GetMethodID(env, gclazz_PlaybackParams.clazz, "setSpeed", "(F)Landroid/media/PlaybackParams;");
    IJK_CHECK_RET(gclazz_PlaybackParams.setSpeed, -1, "missing PlaybackParams.setSpeed");
...
int SDL_Android_AudioTrack_global_init(JNIEnv *env)
{
    ...
    g_clazz.getSampleRate = (*env)->GetMethodID(env, g_clazz.clazz, "getSampleRate", "()I");
    IJK_CHECK_RET(g_clazz.getSampleRate, -1, "missing AudioTrack.getSampleRate");

    g_clazz.getSampleRate = (*env)->GetMethodID(env, g_clazz.clazz, "getSampleRate", "()I");
    IJK_CHECK_RET(g_clazz.getSampleRate, -1, "missing AudioTrack.getSampleRate");

    g_clazz.getPlaybackRate = (*env)->GetMethodID(env, g_clazz.clazz, "getPlaybackRate", "()I");
    IJK_CHECK_RET(g_clazz.getPlaybackRate, -1, "missing AudioTrack.getPlaybackRate");

    g_clazz.setPlaybackRate = (*env)->GetMethodID(env, g_clazz.clazz, "setPlaybackRate", "(I)I");
    IJK_CHECK_RET(g_clazz.setPlaybackRate, -1, "missing AudioTrack.setPlaybackRate");

    if (SDL_Android_GetApiLevel() >= 23){
        g_clazz.getPlaybackParams = (*env)->GetMethodID(env, g_clazz.clazz, "getPlaybackParams", "()Landroid/media/PlaybackParams;");
        IJK_CHECK_RET(g_clazz.getPlaybackParams, -1, "missing AudioTrack.getPlaybackParams");

        g_clazz.setPlaybackParams = (*env)->GetMethodID(env, g_clazz.clazz, "setPlaybackParams", "(Landroid/media/PlaybackParams;)V");
        IJK_CHECK_RET(g_clazz.setPlaybackParams, -1, "missing AudioTrack.setPlaybackParams");
    }

    SDL_Android_Playback_Params_global_init(env);
...
static int audiotrack_get_sample_rate(JNIEnv *env, jobject thiz) {
    ALOGI("[ST] %s", __FUNCTION__);
    int retval = (*env)->CallIntMethod(env, thiz, g_clazz.getSampleRate);
    if ((*env)->ExceptionCheck(env)) {
        ALOGE("[ST] audiotrack_get_sample_rate: write_byte: Exception:");
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionDescribe(env);
            (*env)->ExceptionClear(env);
        }
...
static jobject audiotrack_get_playback_params(JNIEnv *env, jobject thiz) {
    ALOGI("[ST] %s", __FUNCTION__);

    jobject ret_object = NULL;
    jobject local_object = (*env)->CallObjectMethod(env, thiz, g_clazz.getPlaybackParams);
    if ((*env)->ExceptionCheck(env)) {
        ALOGE("audiotrack_get_playback_params: Exception:");
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionDescribe(env);
            (*env)->ExceptionClear(env);
        }
    ...
    ret_object = (*env)->NewGlobalRef(env, local_object);
    if (!ret_object) {
        ret_object = NULL;
        goto fail;
    }
...
static void audiotrack_set_playback_params(JNIEnv *env, jobject thiz, jobject params) {

    ALOGI("[ST] %s", __FUNCTION__);

    (*env)->CallVoidMethod(env, thiz, g_clazz.setPlaybackParams, params);

    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionDescribe(env);
        (*env)->ExceptionClear(env);
        ALOGE("audiotrack_set_playback_params: ");
    }

}

static jobject audiotrack_PlaybackParams_setSpeed(JNIEnv *env, jobject thiz, jfloat speed) {

    ALOGI("[ST] %s %f", __FUNCTION__, speed);

    jobject ret_object = (*env)->CallObjectMethod(env, thiz, gclazz_PlaybackParams.setSpeed, speed);
    if ((*env)->ExceptionCheck(env) || !ret_object) {
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionDescribe(env);
            (*env)->ExceptionClear(env);
        }
        ALOGE("[ST] audiotrack_PlaybackParams_setSpeed: write_byte: Exception:");
        ret_object = NULL;
    }
...
static jint audiotrack_getPlaybackRate(JNIEnv *env, jobject thiz) {
    int retval = (*env)->CallIntMethod(env, thiz, g_clazz.getPlaybackRate);
    if ((*env)->ExceptionCheck(env)) {
        ALOGE("[ST] audiotrack_getPlaybackRate: Exception:");
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionDescribe(env);
            (*env)->ExceptionClear(env);
        }
...
static jint audiotrack_setPlaybackRate(JNIEnv *env, jobject thiz, int sampleRateInHz) {

    ALOGI("[ST] %s %d", __FUNCTION__, sampleRateInHz);

    int retval = (*env)->CallIntMethod(env, thiz, g_clazz.setPlaybackRate, sampleRateInHz);
    if ((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionDescribe(env);
        (*env)->ExceptionClear(env);
        ALOGE("[ST] audiotrack_setPlaybackRate: Exception:");
        return -1;
    }
...
SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_spec(JNIEnv *env, SDL_Android_AudioTrack_Spec *spec)
{
    ...
    case CHANNEL_OUT_5POINT1:
        ALOGI("SDL_Android_AudioTrack: %s", "CHANNEL_OUT_5POINT1");
        break;
    default:
...
void SDL_Android_AudioTrack_setSpeed(JNIEnv *env, SDL_Android_AudioTrack *atrack, float speed)
{
    ...
    if (SDL_Android_GetApiLevel() < 23) {
        jint sample_rate = audiotrack_get_sample_rate(env, atrack->thiz);
        if(sample_rate < 0)
            return;

        audiotrack_setPlaybackRate(env, atrack->thiz, (jint)(sample_rate * speed));

        return;
    }

    jobject temp = NULL;
    jobject params = audiotrack_get_playback_params(env, atrack->thiz);
    if (!params)
        goto fail;

    temp = audiotrack_PlaybackParams_setSpeed(env, params, speed);
    ...
    audiotrack_set_playback_params(env, atrack->thiz, params);

    fail:
    (*env)->DeleteGlobalRef(env, params);
}
