...
static int android_render_rgb_on_rgb(ANativeWindow_Buffer *out_buffer, const SDL_VoutOverlay *overlay, int bpp)
{
    ...
    int min_height = overlay->h - overlay->crop_padding_vertical;
...
//    } else {
        // TODO: 9 padding
        int bytewidth = 0;
//        int bytewidth = overlay->crop ? overlay->w * 2 : IJKMIN(dst_line_size, src_line_size);
        if (overlay->crop && overlay->crop_padding_horizontal > 0) {
            bytewidth = src_line_size - 4 * (overlay->crop_padding_horizontal);
        } else {
            bytewidth = IJKMIN(dst_line_size, src_line_size);
        }
        int offset = 0;
//        int offset = overlay->crop ? overlay->w : 0;
        if (overlay->crop) {
            offset = src_line_size * (overlay->crop_padding_vertical / 2) + overlay->crop_padding_horizontal / 2 * 4;
        } else {
            offset = 0;
        }
//         ALOGE("android_render_rgb_on_rgb (pix-mismatch) %p %d %p %d %d %d", dst_pixels, dst_line_size, src_pixels, src_line_size, bytewidth, min_height);
        av_image_copy_plane(dst_pixels, dst_line_size, src_pixels + offset , src_line_size, bytewidth, min_height);
//    }
...
int SDL_Android_NativeWindow_display_l(ANativeWindow *native_window, SDL_VoutOverlay *overlay)
{
    ...
    int curr_w = overlay->wanted_display_width;
    int curr_h = overlay->wanted_display_height;
    int curr_format = ANativeWindow_getFormat(native_window);
    if (overlay->crop && curr_w > 0 && curr_h > 0) {
        if (overlay->reset_padding) {
            float surface_ratio = curr_w / (float) curr_h;
            float video_ratio = overlay->w / (float) overlay->h;
            if (surface_ratio > video_ratio) {
                overlay->crop_padding_vertical = overlay->h - overlay->w / surface_ratio;
                overlay->crop_padding_horizontal = 0;
                overlay->buff_w = IJKALIGN(overlay->w, 2);
                overlay->buff_h = IJKALIGN(overlay->h - overlay->crop_padding_vertical, 2);
                ALOGI("surface_ratio > video_ratio");
            } else {
                overlay->crop_padding_horizontal = overlay->w - overlay->h * surface_ratio;
                overlay->crop_padding_vertical = 0;
                overlay->buff_w = IJKALIGN(overlay->w - overlay->crop_padding_horizontal, 2);
                overlay->buff_h = IJKALIGN(overlay->h, 2);
                ALOGI("surface_ratio <= video_ratio");
            }
            ALOGI("crop_padding_vertical %d crop_padding_horizontal %d", overlay->crop_padding_vertical, overlay->crop_padding_horizontal);
            overlay->reset_padding = false;
        }
    } else {
        overlay->buff_w = IJKALIGN(overlay->w, 2);
        overlay->buff_h = IJKALIGN(overlay->h, 2);
...
//    ALOGE("crop=%d, surfaceWidth=%d, surfaceHeight=%d, buff_w=%d, buff_h=%d, crop_horizontal=%d, crop_vertical=%d",overlay->crop, curr_w, curr_h, buff_w, buff_h, overlay->crop_padding_horizontal, overlay->crop_padding_vertical);
//    int buf_width = overlay->crop ? overlay->w / 2 : overlay->w;
//    buff_w = IJKALIGN(buf_width, 2);
//    buff_h = IJKALIGN(overlay->h, 2);
    ...
    if (!voutDesc || voutDesc->hal_format != overlayDesc->hal_format) {
        ALOGD("ANativeWindow_setBuffersGeometry: w=%d, h=%d, f=%.4s(0x%x) => w=%d, h=%d, f=%.4s(0x%x)",
            ...
            overlay->buff_w, overlay->buff_h, (char*) &overlay->format, overlay->format);
        retval = ANativeWindow_setBuffersGeometry(native_window, overlay->buff_w, overlay->buff_h, overlayDesc->hal_format);
        if (retval < 0) {
    ...
    if (out_buffer.width != overlay->buff_w || out_buffer.height != overlay->buff_h) {
        ...
        ANativeWindow_setBuffersGeometry(native_window, overlay->buff_w, overlay->buff_h, overlayDesc->hal_format);
        return -1;
    }
