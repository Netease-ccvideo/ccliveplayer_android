...
SDL_VoutOverlay *SDL_Vout_CreateOverlay(int width, int height, Uint32 format, SDL_Vout *vout, int crop, int surface_width, int surface_height)
{
    if (vout && vout->create_overlay)
        return vout->create_overlay(width, height, format, vout, crop, surface_width, surface_height);
...
void SDL_VoutAndroid_DecoderRawData(SDL_Vout *vout, int enable) {
    ...
    if (vout && vout->set_decode_raw_data_enable)
        vout->set_decode_raw_data_enable(vout, enable);
}
